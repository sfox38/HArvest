...
50. **Per-entity `color_scheme` "auto" needs the token-level scheme tracked separately.** A card has three potential scheme sources: page-level (the global default), token-level (set in token settings), and entity-level (overrides per-card). When the entity-level value is `"auto"` it should *defer* to the token-level value, not be ignored. The naive implementation - "only write to `config.colorScheme` when the entity scheme is non-auto" - silently strands the override: setting an entity to "dark" works, setting it back to "auto" leaves `config.colorScheme = "dark"` because the write never happens. The correct shape is to keep `#tokenColorScheme` and `#entityColorScheme` as separate fields, recompute the effective scheme on every change as `entity if explicit, else token`, and write that to the `config` object both when the entity definition arrives *and* when token config arrives. Symptom when missed: switching dark→auto in the panel doesn't repaint the live widget; switching dark→light does, because both are explicit writes.
...
52. **Popover API (`popover="manual"`) is the only reliable way to escape shadow DOM stacking contexts.** Drop-down menus inside an `<hrv-card>` shadow root are clipped or painted-under by ancestor cards because the host card is its own stacking context (`isolation: isolate`) *and* HA's powering animations leave a permanent `transform: scale(1)` on `[part=card]`, which converts every descendant `position: fixed` into being positioned relative to the card. Raising `z-index` on the host helps for sibling cards on the same page but does nothing against arbitrary HTML on the hosting site. The fix: render the dropdown with `popover="manual"`, position it via `position: fixed` + inline `left/top/width` computed from `triggerEl.getBoundingClientRect()`, and call `dropdown.showPopover()` / `dropdown.hidePopover()` to toggle. Popover elements live in the browser's *top layer*, which is rendered above every stacking context in the entire document. Combine with the lesson #34 drop-up detection (compute `spaceBelow` / `spaceAbove`, choose the larger). Re-position on `scroll` (capture phase) and `resize` while open. Browser support is broad (Chrome 114+ / Safari 17+ / Firefox 125+).
